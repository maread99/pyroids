...
class EndLabels(WindowLabels):
    ...
    def add_labels(self):
        ...
        self._winner_lbl = self.add_label(
            "placeholder", font_size=60, y=self.labels[0].y + 110, bold=True
        )
    ...
    def set_labels(self, winner: Union["red", "blue", bool, None], completed=False):
...
class InstructionLabels(WindowLabels):
    ...
    def add_labels(self):
        ...
        right_kwargs.update(
            {"x": self._x + x_from_center, "anchor_x": "left", "width": 150}
        )
        ...
        self.add_label(
            "".join([control[0] + "\n" for control in controls]), y=y, **left_kwargs
        )
...
class StockLabel(pyglet.text.layout.TextLayout):
    ...
    class StockLabelElement(pyglet.text.document.InlineElement):
        ...
        def place(self, layout, x: int, y: int):
            ...
            self._sprite = Sprite(
                self.image, x, y, batch=layout.batch, group=layout.top_group
            )
            self._sprite.draw()
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """
        assert group is None or isinstance(group, pyglet.graphics.OrderedGroup)
