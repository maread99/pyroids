...
def load_image(
    filename: str, anchor: Union["origin", "center"] = "origin"
) -> TextureRegion:
...
def anim(
    filename, rows: int, cols: int, frame_duration: float = 0.1, loop=True
) -> Animation:
...
def vector_anchor_to_rotated_point(
    x: int, y: int, rotation: Union[int, float]
) -> Tuple[int, int]:
...
class SpriteAdv(Sprite, StaticSourceMixin):
    ...
    def _end_lives_selective(
        cls, exceptions: Optional[List[Sprite]] = None, kill=False
    ):
    ...
    def __init__(
        self,
        scale_to: Union[Sprite, Texture] = None,
        sound=True,
        sound_loop=False,
...
class PhysicalSprite(SpriteAdv):
    ...
    def __init__(
        ...
        cruise_speed=200,
        rotation_cruise_speed=200,
        initial_rotation=0,
        at_boundary: Optional[str] = None,
        **kwargs,
    ):
        ...
        ++at_boundary++ will override, for this instance, any default
            ...
            'stop', 'die' or 'kill'.
        """
        assert self._setup_complete, (
            "PhysicalSprite class must be setup before instantiating instances"
        )
        super().__init__(**kwargs)
    ...
    def _random_x(self, exclude_border: Optional[int] = None) -> int:
        ...
        return random.randint(self.X_MIN + exclude_border, self.X_MAX - exclude_border)
    ...
    def _random_y(self, exclude_border: Optional[int] = None) -> int:
        ...
        if exclude_border is None:
            exclude_border = self._default_exclude_border()
        return random.randint(self.Y_MIN + exclude_border, self.Y_MAX - exclude_border)
...
class PhysicalSpriteInteractive(PhysicalSprite):
    ...
    @staticmethod
    def _eval_keymod(
        key: Union[int, "num", "numrow", "numpad"], modifiers: Union[int, str] = ""
    ) -> str:
    ...
    def add_keymod_handler(
        ...
        if while_pressed is not None:
            assert modifiers == "" and not (
                isinstance(key, str) and key[:3] == "num"
            ), (
    ...
    def _keymod_handled(
        self, key: int, modifiers: Union[int, str] = ""
    ) -> Union[str, bool]:
    ...
    def _execute_any_key_handler(
        self, key: int, circumstance: str, modifiers: Union[int, str] = ""
    ):
