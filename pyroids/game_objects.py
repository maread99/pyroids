...
class Bullet(Ammunition, PhysicalSprite):
    ...
    def __init__(self, control_sys, *args, **kwargs):
        ...
        super().__init__(
            *args, initial_rotation_speed=0, rotation_cruise_speed=0, **kwargs
        )
...
class Weapon(object):
    ...
    ammo_cls = {"blue": Bullet, "red": BulletRed}

    fire_when_shield_up = False

    def __init__(self, control_sys, initial_stock: int = 0, max_stock: int = 9):
        """
...
class HighVelocityCannon(Weapon):
    ...
    ammo_cls = {"blue": BulletHighVelocity, "red": BulletHighVelocityRed}
...
class FireworkLauncher(Weapon):
    ...
    ammo_cls = {"blue": Firework, "red": FireworkRed}

    def __init__(
        self,
        *args,
        dflt_explosion_distance=200,
        dflt_num_bullets: Optional[int] = None,
        **kwargs,
    ):
        """
...
class SLD_Launcher(Weapon):
    ...
    ammo_cls = {"blue": SuperLaserDefence, "red": SuperLaserDefenceRed}

    def __init__(self, *args, dflt_num_bullets: Optional[int] = None, **kwargs):
        """
...
class MineLayer(Weapon):
    ...
    ammo_cls = {"blue": Mine, "red": MineRed}
...
class ShieldGenerator(Weapon):
    ...
    ammo_cls = {"blue": Shield, "red": ShieldRed}
...
class PickUp(PhysicalSprite):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.ammo_sprite = Sprite(
            ammo_img, self.x, self.y, batch=self.batch, group=self.group
        )
...
class ControlSystem(object):
    ...
    ShipCls = {"blue": Ship, "red": ShipRed}

    _RadiationMonitorCls = {"blue": RadiationMonitor, "red": RadiationMonitorRed}
    ...
    def process_pickup(self, pickup):
        ...
        system.
        """
        assert pickup.color == self.color, (
            "Pickup color should be the same as color of control system."
        )
    ...
    def ammo_base_kwargs(self) -> dict:
        ...
        """
        ship = self.ship
        kwargs = {"x": ship.x, "y": ship.y, "batch": ship.batch, "group": ship.group}
        return kwargs
    ...
    def bullet_kwargs(self, margin: Optional[int] = None, **kwargs):
        ...
        """
        assert False not in [kwarg not in kwargs for kwarg in ["x", "y", "batch"]]
